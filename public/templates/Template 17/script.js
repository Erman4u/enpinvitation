document.addEventListener('DOMContentLoaded', () => {
  ...
  const defaultWishes = [
    {
      ...
      guestCount: "1 Orang",
      status: "Hadir",
      wish: "Selamat ya Mas Bagus dan Mbak Sekar! Semoga menjadi keluarga yang sakinah, mawaddah, warahmah. Ikut bahagia melihat kalian berdua.",
  ...
  const createPetal = () => {
    ...
    // Random properties
    const size = Math.random() * 15 + 10; // width between 10px and 25px
    const left = Math.random() * 100; // left position 0 - 100vw
    const duration = Math.random() * 10 + 6; // fall duration between 6s and 16s
    const delay = Math.random() * 5; // delay up to 5 seconds
    const blur = Math.random() * 1.5; // subtle depth of field blur

    petal.style.width = `${size}px`;
    // Shape styling: romantic leaf petal shape
    petal.style.height = `${size * (Math.random() * 0.4 + 0.8)}px`; 
    ...
    // Color gradient variations: shades of Emerald Green and Elegant Gold
    const colors = [
      'rgba(4, 120, 87, 0.25)',   // emerald green
      'rgba(6, 78, 59, 0.2)',     // deep emerald
      'rgba(212, 175, 55, 0.22)',  // soft gold
      'rgba(251, 246, 238, 0.3)'   // champagne
    ];
    petal.style.backgroundColor = colors[Math.floor(Math.random() * colors.length)];

    petalsContainer.appendChild(petal);

    // Remove from DOM after animation completes to avoid memory bloat
    setTimeout(() => {
      petal.remove();
    }, (duration + delay) * 1000);
  };
  ...
  const startFallingPetals = () => {
    // Immediately spawn a few petals for immediate visual effect
    for (let i = 0; i < 8; i++) {
      createPetal();
    }
